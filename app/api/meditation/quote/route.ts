...
function buildExistingMeditationFilter(
  wallet: string,
  fid: number | null,
): string {
  const filters = [
...
async function fetchQuoteWithRetry(
  ...
  for (
    ...
    try {
      ...
      lastMessage =
        raw.trim()
          ? `Quote provider rejected the stillness session: ${raw}`
          : `Quote provider rejected the stillness session with status ${response.status}.`;
  ...
  try {
    ...
    if (existing) {
      return NextResponse.json(
        {
          error:
            'Today’s stillness session is already complete.',
        },
        {
          status: 409,
          headers:
    ...
    console.error(
      'GET /api/meditation/quote failed:',
      cause,
    );
    ...
    const message =
      cause instanceof Error
        ? cause.message
        : 'Unable to quote stillness session.';
